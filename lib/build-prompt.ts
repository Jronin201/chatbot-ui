...
export async function buildFinalMessages(
  ...
  const processedChatMessages = chatMessages.map((chatMessage, index) => {
    ...
    if (nextChatMessageFileItems.length > 0) {
      const findFileItems = nextChatMessageFileItems
        ...
        .filter(item => item !== undefined) as Tables<"file_items", never>[]
  ...
  let tempSystemMessage: Tables<"messages", never> = {
    chat_id: "",
    assistant_id: null,
...
function buildRetrievalText(fileItems: Tables<"file_items", never>[]) {
