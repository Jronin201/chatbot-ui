...
export class EventTriggerSystem {
  ...
  private updateEntity(
    ...
    // Update NPCs
    if (updatedData.keyNPCs) {
      const npcIndex = updatedData.keyNPCs.findIndex(npc => npc.id === entityId)
      if (npcIndex !== -1) {
        updatedData.keyNPCs[npcIndex] = {
          ...updatedData.keyNPCs[npcIndex],
          ...changes
        }
      }
  ...
  private createTimelineEvent(
    ...
    const newEvent: TimelineEvent = {
      ...
      title: eventData.title || "Auto-generated Event",
      description:
        eventData.description || "An event occurred during the campaign",
      participants: eventData.participants || [],
      location: eventData.location || "",
      consequences: eventData.consequences || [],
      gameImpact: eventData.gameImpact || {}
    }

    updatedData.timeline.push(newEvent)
